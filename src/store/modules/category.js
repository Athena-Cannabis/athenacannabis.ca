...
import ProductCategoryService from '../../services/woocommerce/category';
...
const getters = {
  ...
  // Get Cannabis sub categories
  getCannabisCategories (state, getters ) {

    var categories = [];

    // Find the index of the Cannabis category
    const findCannabisIndex = getters.getCategories.findIndex(element => element.title === 'Cannabis');

    if (findCannabisIndex != -1 ) {
      categories = getters.getChildCategoriesById( getters.getCategories[findCannabisIndex].id );
    }
  ...
  // Get Accessories sub categories
  getAccessoryCategories (state, getters ) {

    var categories = [];

    // Find the index of the Cannabis category
    const findCannabisIndex = getters.getCategories.findIndex(element => element.title === 'Accessories');
  ...
  getChildCategoriesById: (state, getters) => (id) => {
    ...
    categories = getters.getCategories.filter(element => element.parent === id);
